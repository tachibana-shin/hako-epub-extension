export default defineRegistry({
  ...
  lang: "vi",
  findAuthor: () => $("a", contains("div.book__detail-text", "Tác giả:"))?.textContent.trim(),
  findBlocks: ".book__detail-name",
  findTarget: () => $("#chapter-list-container")!,
  extractCover: () => $(".book__detail-image img")?.getAttribute("src") ?? undefined,
  findTags: () =>
    Array.from($$("a", contains(".book__detail-text", "Tag:"))).map((a) => a.textContent?.trim()),
  title: () => {
    const list = Array.from($$("#chapter-list-container .list-chapter__name > a"))
