export default defineRegistry({
  ...
  lang: "vi",
  findAuthor: () =>
    Array.from($$("a", contains("dt", "Tác giả")?.nextElementSibling)).map((a) =>
      a.textContent.trim()
    ),
  findBlocks: "h2.card-title",
  findTarget: () => $("#listChapters")!,
  extractCover: () => $(".card img.img-fluid")?.getAttribute("src") ?? undefined,
  ...
  targetQueries: {
    ...
    container: ($) => $("div.affActive").html() || $("#chapter-content-render").html()
  },
  cleaner: ($) => {
